/**
 * Adiciona colunas da tela Empenhos Pendentes em public.empenho.
 * Uso (na pasta backend): npm run db:fix-empenho-columns
 */
...
async function main() {
  const sqlPath = path.join(__dirname, '../prisma/scripts/add-missing-empenho-columns.sql');
  const sql = fs.readFileSync(sqlPath, 'utf8');
  const client = new Client(getClientConfig());
  try {
    await client.connect();
    await client.query(sql);
    console.log('Colunas de public.empenho verificadas/adicionadas com sucesso.');
  } catch (err) {
