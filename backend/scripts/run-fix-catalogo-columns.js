/**
 * Adiciona colunas faltantes em ctrl.safs_catalogo (descricao, resp_controle, setor, unidade, created_at, updated_at).
 * Uso (na pasta backend): npm run db:fix-catalogo-columns
...
async function main() {
  const sqlPath = path.join(__dirname, '../prisma/scripts/add-missing-safs-catalogo-columns.sql');
  const sql = fs.readFileSync(sqlPath, 'utf8');
  const client = new Client(getClientConfig());
  try {
    await client.connect();
    await client.query(sql);
    console.log('Colunas de ctrl.safs_catalogo verificadas/adicionadas com sucesso.');
  } catch (err) {
